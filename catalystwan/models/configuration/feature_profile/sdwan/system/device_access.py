...
class SourceIPPrefix(BaseModel):
    ...
    source_ip_prefix_list: Union[Global[List[IPv4Interface]], Variable] = Field(
...
class DestinationIPPrefix(BaseModel):
    ...
    destination_ip_prefix_list: Union[Global[List[IPv4Interface]], Variable] = Field(
...
class Sequence(BaseModel):
    ...
    def match_destination_data_prefix_variable(self, variable_name: str) -> Variable:
        prefix_var = as_variable(variable_name)
...
class DeviceAccessIPv4Parcel(_ParcelBase):
    model_config = ConfigDict(extra="forbid", populate_by_name=True)

    type_: Literal["ipv4-device-access-policy"] = Field(default="ipv4-device-access-policy", exclude=True)
